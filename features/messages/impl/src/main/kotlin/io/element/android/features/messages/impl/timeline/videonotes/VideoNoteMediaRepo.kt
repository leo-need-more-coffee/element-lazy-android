...
@AssistedInject
class DefaultVideoNoteMediaRepo(
    ...
    @AssistedFactory
    fun interface Factory : VideoNoteMediaRepo.Factory {
        override fun create(
            mediaSource: MediaSource,
            @Assisted mimeType: String?,
            @Assisted filename: String?,
        ): DefaultVideoNoteMediaRepo
    }
    ...
    private val cachedFile: File? = mxcTools.mxcUri2FilePath(mediaSource.safeUrl)?.let {
        File("${cacheDir.path}/$CACHE_VIDEO_NOTE_SUBDIR/$it")
    }
}

private const val CACHE_VIDEO_NOTE_SUBDIR = "temp/video_note"
